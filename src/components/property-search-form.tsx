'use client';

import { Button } from "@/components/ui/button";
import { Input } from "@/components/ui/input";
import { SlidersHorizontal, Search, Building2, Home, Trees, Building, ShoppingCart, DollarSign } from "lucide-react";

// In a real app, this would be dynamically generated by an AI based on user behavior
const quickFilters = [
    { label: 'Flats', icon: Building2 },
    { label: 'Houses', icon: Home },
    { label: 'Plots', icon: Trees },
    { label: 'Commercial', icon: Building },
    { label: 'For Sale', icon: DollarSign },
    { label: 'For Rent', icon: ShoppingCart },
];


export function PropertySearchForm() {
  return (
    <div className="space-y-4 my-6">
      <div className="flex gap-2 items-center">
        <div className="relative flex-grow">
          <Search className="absolute left-3 top-1/2 -translate-y-1/2 h-5 w-5 text-muted-foreground" />
          <Input id="location" placeholder="Search for properties in Pithampur..." className="pl-10 h-12 text-base" />
        </div>
        <Button variant="outline" size="icon" className="h-12 w-12 flex-shrink-0">
          <SlidersHorizontal className="h-5 w-5" />
           <span className="sr-only">Filters</span>
        </Button>
      </div>
      <div className="flex gap-2 overflow-x-auto pb-2 -mx-4 px-4">
        {quickFilters.map(filter => (
           <Button key={filter.label} variant="outline" className="rounded-full flex-shrink-0 flex items-center gap-2">
             <filter.icon className="h-4 w-4 text-primary" />
             <span>{filter.label}</span>
           </Button>
        ))}
      </div>
    </div>
  );
}
